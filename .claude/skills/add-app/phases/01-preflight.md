...
4. **Collision check.** Test for existence of `apps/<name>/` and `inactive_apps/<name>/`. If either exists, write `preflight.json` with `mode: "reeval"` and hand off to `phases/reeval.md`. Otherwise write `mode: "new"` and continue to `phases/02-research.md`.
